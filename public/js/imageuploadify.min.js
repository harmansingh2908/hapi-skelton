(function($, window, document, undefined) {
  ...
  $.fn.imageuploadify = function(opts) {
    ...
    this.each(function() {
      ...
      let totalFiles = [];
      let counter = 0;
      let dragbox = $(`<div class="imageuploadify "><div class="imageuploadify-overlay"></div><div class="imageuploadify-images-list"><button type="button"class="btn btn-default"><i class="fa fa-plus" aria-hidden="true"></i>
</button></div></div>`);
      ...
      const retrieveFiles = files => {
        for (let index = 0; index < files.length; ++index) {
          if (
            !accept ||
            compareMimeType(result, files[index].type, /(?:\.([^.]+))?$/.exec(files[index].name)[1])
          ) {
      ...
      const readingFile = (id, file) => {
        ...
        let container1 = $(
          `<div class='imageuploadify-container1'><button type='button'class='btn'><i class="fa fa-times" aria-hidden="true"></i></button><div class='imageuploadify-details'><span>${file.name}</span><span>${file.type}</span><span>${file.size}</span></div></div>`
        );
        let details = container1.find('.imageuploadify-details');
        let deleteBtn = container1.find('button');
        container1.css('margin-left', marginSize + 'px');
        details
          .hover(function() {
            $(this).css('opacity', '1');
          })
        ...
        if (file.type && file.type.search(/image/) != -1) {
          fReader.onloadend = function(e) {
            ...
            imagesList.append(container1);
            imagesList
              .find('.imageuploadify-container1:nth-child(' + boxesNb + 'n+4)')
              .css('margin-left', marginSize + 'px');
            imagesList
              .find('.imageuploadify-container1:nth-child(' + boxesNb + 'n+3)')
      ...
      $(window).bind('resize', function(e) {
        ...
        $(window).resize(function() {
          ...
          window.resizeEvt = setTimeout(function() {
            ...
            const boxesNb = Math.floor(width / 100);
            const marginSize = Math.floor((width - boxesNb * 100) / (boxesNb + 1));
            let container1s = imagesList.find('.imageuploadify-container1');
            for (let index = 0; index < container1s.length; ++index) {
              $(container1s[index]).css('margin-right', '0px');
              $(container1s[index]).css('margin-left', marginSize + 'px');
            }
            imagesList
              .find('.imageuploadify-container1:nth-child(' + boxesNb + 'n+4)')
              .css('margin-left', marginSize + 'px');
